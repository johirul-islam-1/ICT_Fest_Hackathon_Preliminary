...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    ...
    _pricing_warmup()
    for b in existing:
        # Strict boundary checks: existing.start < new.end AND new.start < existing.end
        if b.start_time < end and start < b.end_time: #fix9
            return True
    return False
...
def create_booking( #fix 9
    payload: BookingCreateRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    with _booking_lock: # Encapsulate booking validation and DB persistence atomically
        ...
        if start <= now: # No grace window of any size
        ...
        # Bounds verification including minimum bounds and negative boundaries
        ...
        cache.invalidate_report(user.org_id) # Invalidate usage report cache on booking creation
...
def get_booking(
    ...
    response["start_time"] = iso_utc(booking.start_time) # Fix 7
    response["refunds"] = [
        {
